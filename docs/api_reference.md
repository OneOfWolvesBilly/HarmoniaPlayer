...
**Persistence semantics:** only a named preset persists its curve. When `currentPresetName` is `nil` (custom state), `persist()` writes a flat curve and preamp, so an unsaved edit is session-only and does not survive a relaunch. Together with the init resolution above, a persisted preset name that no longer resolves can never restore a phantom picker selection or a stale curve.

**Init side effect:** the constructor resolves the loaded `currentPresetName` against the available presets — a resolvable name derives its curve from that preset (so the picker label always matches the live curve), while a nil or no-longer-resolvable name falls back to a flat custom state (`currentPresetName = nil`, flat band gains and preamp). It then pushes the resolved state to the injected `EQService` (`setEnabled`, `setPreamp`, `setBandGains`) so the audio chain matches the coordinator's published state from t=0.
