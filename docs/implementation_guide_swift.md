...
## 4. Sandbox File Access (Slice 9-M)

HarmoniaPlayer ships with `ENABLE_APP_SANDBOX = YES`. This section documents the two patterns the codebase uses for file access under sandbox.

### 4.1 Track Bookmark — Security-Scoped

`Track` persists URLs as security-scoped bookmarks so that imported tracks remain accessible across cold-launch.

**Encode (Track.swift, generated by JSONEncoder):**
...
The bookmark capture must happen while an active sandbox extension exists on `url`. `AppState.load(urls:)` ensures this by wrapping each URL with `startAccessingSecurityScopedResource()` / `stopAccessingSecurityScopedResource()` (paired via `defer` inside the `for` loop body — Swift `defer` is per-scope, so the loop body is the relevant scope and the pair fires per-iteration).

**Decode (Track.swift, JSONDecoder init):**
...
if let bookmark = try c.decodeIfPresent(Data.self, forKey: .accessBookmark) {
    ...
    if let resolved = try? URL(
        ...
        url = resolved
        // Hold the security-scoped extension for the URL's lifetime.
        // Calling stopAccessingSecurityScopedResource() after verification
        // releases the extension and makes every subsequent
        // PlaybackService open / TagReader read fail with FigFile err=-12203.
        let started = resolved.startAccessingSecurityScopedResource()
        isAccessible = started
        // stale flag is captured but no explicit refresh helper:
        // url is updated to the resolved value, so the next encode
        // pass regenerates a fresh bookmark from the current URL.
    } else {
        // Falls through to urlPath path with isAccessible = false.
        // Catches legacy .minimalBookmark bytes from any pre-9-M build —
        // those bytes lack security scope and fail this resolve.
        ...
    }
}
```

### 4.2 Sibling Read — Related Items + NSFileCoordinator

For sibling reads (e.g. `.lrc` next to `.flac`), Apple's Related Items mechanism is required. See `LyricsService.swift` for the production usage:
...
case .lrc:
    ...
    guard let data = readData else { throw LyricsServiceError.decodingFailed }
    // ... encoding detect + String(data:encoding:) ...
```

`SiblingFilePresenter` (`Shared/Services/SiblingFilePresenter.swift`) is a minimal `NSFilePresenter` conformance — three properties and a constructor, no behavioural callbacks needed for read-only sibling access:
...
**Info.plist requirement:** the sibling extension must be declared with `NSIsRelatedItemType=YES` and `CFBundleTypeRole=Editor` in `CFBundleDocumentTypes`. Without this, `NSFileCoordinator.addFilePresenter` silently fails and reads error with `Code=257`. See `App/HarmoniaPlayer/HarmoniaPlayer/Info.plist`.

### 4.3 Error Categorisation for UI

`LyricsPanel` uses `lyricsErrorMessageKey(for: Error)` (free function in `LyricsService.swift`) to map errors to localisable keys:
...
`LyricsPanel` calls it from the catch path:
...
`.noEmbeddedLyrics` and `.sidecarNotFound` are not categorised here because the surrounding `LyricsPanel.reload()` flow short-circuits via `LyricsResolution.hasAny == false` before either error can be thrown.

---

## 5. IAPManager Implementation
...
### 6.2 Calling Async AppState Methods from SwiftUI
...
### 6.3 Seeking from a Slider Binding
